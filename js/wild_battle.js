...
// ユーザーがどこでポケモンとエンカウントしたかを、localstorageにあるデータからとってくる
const whereuser = localStorage.getItem("whereuser");
var opp_level = 1;
var opp_party = [];
// エンカウントした場所に応じて、出現するポケモンの配列を持ってくる
if (whereuser === "grass") {
    decideOpp(grasspokemontable);
} else if (whereuser === "rock") {
    decideOpp(rockpokemontable);
} else if (whereuser === "blackhole") {
    decideOpp(legendpokemontable);
}

// 出現する可能性のあるポケモンの中から、ランダムに選ぶ＆ランダムなレベルにする
function decideOpp(table) {
    const keys = Object.keys(table);
    const index = Math.floor(Math.random() * keys.length);
    const wildpokemon_id = keys[index]
    opp_level = Math.floor(Math.random() * (21)) + 30;
    opp_party = [duplicate(table[wildpokemon_id], opp_level, HPstatusCalculation(table[wildpokemon_id].maxhp, opp_level))];
}

// localstorageにあるユーザーのポケモンの手持ちのデータから復元をする関数
...
// userPartyは手持ちで、userPokeboxはポケモンボックスに登録されているポケモンの情報
...
if (savedUserParty !== null && savedUserParty.length > 0) {
    ...
    // もし手持ちのポケモンのデータがないのなら、フシギダネlv.50を代わりに手持ちに加える様に設定
    var user_party = [duplicate(bulbasaurTemplate, 50, HPstatusCalculation(bulbasaurTemplate.maxhp, 50))];
    var userParty = [
        { id: "bulbasaur", lv: 50, hp: HPstatusCalculation(bulbasaurTemplate.maxhp, 50) }
    ]
}
var movebuttons = [
...
// 図鑑（pokemon.js)にあるポケモンのデータと、保存されているレベルとhpのデータから
// 、手持ちのポケモンの配列と、敵のデータを復元する関数
...
// 敵が行動（ランダムな技を使う）時の関数。主に道具を使った後に使う様に設定したい
function oppmove() {
...
// ポケモンをボールを使って捕まえる時の関数。ballRatioがボールごとの捕獲率でoppは捕まえたいポケモン
function pokecatch(ballRatio, opp) {
    $(".tool").addClass("hidden");
    // まず相手のポケモンの姿を隠す
    var enemy = document.getElementById("opppokemon");
    enemy.classList.add("hidden");
    // ポケモンごとに捕獲難易度（どれだけ捕まえやすいか）のデータがあるので、それとボールの捕獲難易度をかけている
    var a = (3 * opp.maxhp - 2 * opp.hp) * opp.catchrate * ballRatio / 3 / opp.maxhp;
    // もしaの値が大きすぎるなら上限の255に設定する（255なら確定で捕まえることができる）
    if (a > 255) {
        a = 255;
    }
    // ポケモンの捕獲の確率を定義
    const catchratio = a / 255;
    console.log(catchratio);
    setTimeout(() => {
        if (Math.random() <= catchratio) {
            // 手持ちのポケモンに加えることができるかどうかを判定
